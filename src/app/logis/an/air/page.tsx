...
export default function ANAirListPage() {
  ...
  const handleEdit = () => {
    ...
    router.push(`/logis/an/air/register?id=${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch('/api/an/air', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: selectedRows }) });
    ...
    <PageLayout title="도착통지 목록 (A/N) - 항공" subtitle="Logis > 항공수입 > 도착통지 목록" onClose={handleCloseClick}>
      <main ref={formRef} className="p-6">
        <div className="flex justify-between items-center mb-6">
          <button onClick={() => router.push('/logis/an/air/register')} className="px-4 py-2 bg-[var(--surface-100)] border border-[var(--border)] rounded-lg hover:bg-[var(--surface-200)] text-sm">신규</button>
          ...
          {loading ? (
            ...
            <table className="table">
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {sortedList.length === 0 ? (
                  <tr><td colSpan={16} className="px-4 py-8 text-center text-[var(--muted)]">데이터가 없습니다.</td></tr>
