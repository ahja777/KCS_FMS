...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  ACCEPTED: { label: '승인', color: 'bg-green-500' },
  REJECTED: { label: '거부', color: 'bg-red-500' },
  HOLD: { label: '보류', color: 'bg-yellow-500' },
};

export default function AMSSeaDetailPage() {
  ...
  const [data, setData] = useState<AMSDetailData | null>(null);
  const [isEditing, setIsEditing] = useState(false);
  const [editData, setEditData] = useState<AMSDetailData | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/ams/sea?amsId=${params.id}`);
        if (response.ok) {
  ...
  const handleChange = (field: keyof AMSDetailData, value: string | number) => {
    if (editData) setEditData({ ...editData, [field]: value });
  };

  const handleDelete = async () => {
    if (!data) return;
    if (confirm('이 AMS를 삭제하시겠습니까?')) {
      try {
        const response = await fetch(`/api/ams/sea?ids=${data.id}`, { method: 'DELETE' });
        if (response.ok) {
          alert('AMS가 삭제되었습니다.');
  ...
  if (!data) return <div className="min-h-screen bg-[var(--background)] flex items-center justify-center">AMS를 찾을 수 없습니다.</div>;
        ...
        <main ref={formRef} className="p-6">
          <div className="flex justify-end items-center mb-6">
            <div className="flex gap-2">
              <button onClick={() => router.push('/logis/ams/sea')} className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">목록</button>
              {isEditing ? (
                <>
                ...
                <div>
                  ...
                  {isEditing ? (
                    <select value={displayData.status || ''} onChange={e => handleChange('status', e.target.value)} className="w-full h-[38px] px-3 bg-[var(--surface-50)] border border-[var(--border)] rounded-lg">
                      <option value="DRAFT">작성중</option><option value="SUBMITTED">전송</option><option value="ACCEPTED">승인</option><option value="REJECTED">거부</option><option value="HOLD">보류</option>
                    </select>
                  ) : (
