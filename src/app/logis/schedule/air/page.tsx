...
export default function AirSchedulePage() {
  ...
  const [data, setData] = useState<AirScheduleData[]>([]);
  const [loading, setLoading] = useState(false);

  const [selectedRows, setSelectedRows] = useState<number[]>([]);

  const { sortConfig, handleSort, sortData, getSortStatusText, resetSort } = useSorting<AirScheduleData>();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/schedule/air?${params}`);
      if (response.ok) {
  ...
  const handleRowClick = (id: number) => {
    router.push(`/logis/schedule/air/${id}`);
  };
  // 수정 버튼 핸들러
  const handleEdit = () => {
    ...
    router.push(`/logis/schedule/air/register?id=${id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/schedule/air?ids=${selectedRows.join(',')}`, { method: 'DELETE' });
      ...
      <main ref={formRef} className="p-6">
        <div className="flex justify-between items-center mb-6">
          <div className="flex gap-2">
            <button onClick={() => router.push('/logis/schedule/air/register')} className="px-4 py-2 bg-[var(--surface-100)] border border-[var(--border)] rounded-lg hover:bg-[var(--surface-200)] text-sm">신규</button>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => router.push('/logis/schedule/air/register')}
            ...
            <table className="table">
              <thead>
                <tr>
                  ...
                  <SortableHeader columnKey="carrierName" label="항공사" sortConfig={sortConfig} onSort={handleSort} />
                  <SortableHeader columnKey="flightNo" label="편명" sortConfig={sortConfig} onSort={handleSort} />
                  <SortableHeader columnKey="aircraftType" label="기종" sortConfig={sortConfig} onSort={handleSort} />
                  <SortableHeader columnKey="origin" label="Origin" sortConfig={sortConfig} onSort={handleSort} />
                  <SortableHeader columnKey="destination" label="Dest" sortConfig={sortConfig} onSort={handleSort} />
                  ...
                  <SortableHeader columnKey="transitHours" label="T/T" sortConfig={sortConfig} onSort={handleSort} />
                  <SortableHeader columnKey="frequency" label="운항" sortConfig={sortConfig} onSort={handleSort} />
                  <th>Cut-Off</th>
                ...
                {loading ? (
                  ...
                  sortedList.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="text-center font-medium">{item.carrierName || item.carrierId || '-'}</td>
                      <td className="text-center">{item.flightNo || '-'}</td>
                      <td className="text-center">{item.aircraftType || '-'}</td>
                      ...
                      <td className="text-center">
                        {item.destination || '-'}
                        {item.destTerminal && (
                          <><br /><span className="text-[var(--muted)] text-xs">{item.destTerminal}</span></>
                        )}
                      </td>
                      <td className="text-center">{item.etd || '-'}</td>
                      <td className="text-center">{item.eta || '-'}</td>
                      <td className="text-center">
                        {item.transitHours ? `${item.transitHours}h` : '-'}
                      </td>
                      <td className="text-center">
