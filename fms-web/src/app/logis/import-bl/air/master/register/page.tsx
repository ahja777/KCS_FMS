...
const initialMainData: MainData = {
  ioType: 'IN', jobNo: '', bookingNo: '', mawbNo: '',
  obDate: '', arDate: '',
...
function ImportMasterAWBRegisterContent() {
  ...
  const handleConfirmClose = () => { setShowCloseModal(false); router.push('/logis/import-bl/air/master'); };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setMainData({
          ioType: d.IO_TYPE || 'IN', jobNo: d.JOB_NO || '', bookingNo: d.BOOKING_NO || '', mawbNo: d.MAWB_NO || '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setIsSaved(true);
        alert('저장되었습니다.');
        if (!editId) router.replace(`/logis/import-bl/air/master/register?id=${mawbId}`);
  ...
  const handleList = () => router.push('/logis/import-bl/air/master');
  const handleNewHouse = () => {
    if (editId) router.push(`/logis/import-bl/air/house/register?masterId=${editId}`);
            ...
            <div className="grid grid-cols-6 gap-3">
              ...
              <div>
                ...
                <select value={mainData.bizType} onChange={e => handleMainChange('bizType', e.target.value)} className={selectCls}>
                  <option value="">선택</option><option value="수입">수입</option><option value="수입현적">수입현적</option><option value="삼국간">삼국간</option>
                </select>
              </div>
              <div>
            ...
            <div className="w-[30%] space-y-3">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="flex-1"><span className="block text-[10px] text-[var(--muted)] mb-0.5">P (Prepaid)</span><input type="number" step="0.01" value={cargoData.weightChargeP} onChange={e => handleCargoChange('weightChargeP', parseFloat(e.target.value) || 0)} className={numCls} /></div>
                  <div className="flex-1"><span className="block text-[10px] text-[var(--muted)] mb-0.5">C (Collect)</span><input type="number" step="0.01" value={cargoData.weightChargeC} onChange={e => handleCargoChange('weightChargeC', parseFloat(e.target.value) || 0)} className={numCls} /></div>
                </div>
              </div>
              <div>
                ...
                <tbody>
                  {cargoData.otherCharges.length === 0 ? (
                    ...
                    <tr key={c.id} className="border-b border-[var(--border)]">
                      ...
                      <td className="p-1"><select value={c.pcType} onChange={e => updateOtherCharge(c.id, 'pcType', e.target.value)} className={selectCls}><option value="P">P</option><option value="C">C</option></select></td>
                      <td className="p-1"><select value={c.acType} onChange={e => updateOtherCharge(c.id, 'acType', e.target.value)} className={selectCls}><option value="A">Agent</option><option value="C">Carrier</option></select></td>
                      <td className="p-1 text-center"><button type="button" onClick={() => removeOtherCharge(c.id)} className="text-red-400 hover:text-red-500"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button></td>
                    </tr>
                  ))}
                </tbody>
                {cargoData.otherCharges.length > 0 && (
