...
const STATUS_OPTIONS = [
  ...
  { value: 'REQUESTED', label: '요청', color: 'bg-blue-500' },
  { value: 'APPROVED', label: '승인', color: 'bg-green-500' },
  { value: 'PROCESSING', label: '처리중', color: 'bg-yellow-500' },
...
const sampleOrders: CustomerOrder[] = [
  { id: 1, co_number: 'CO20260125-0001', order_type_code: 'EXP-FCL', order_type_name: 'Export FCL', biz_type: 'FORWARDING', customer_code: 'SAMSUNG', customer_name: '삼성전자', shipper_name: 'Samsung Electronics', consignee_name: 'Samsung America', pol: 'KRPUS', pod: 'USLAX', etd: '2026-01-28', eta: '2026-02-15', cargo_type: 'FCL', commodity: '전자제품', quantity: 100, weight: 25000, volume: 120, incoterms: 'FOB', status: 'APPROVED', remarks: '', created_at: '2026-01-25' },
  { id: 2, co_number: 'CO20260125-0002', order_type_code: 'IMP-LCL', order_type_name: 'Import LCL', biz_type: 'FORWARDING', customer_code: 'LG', customer_name: 'LG전자', shipper_name: 'LG Japan', consignee_name: 'LG Electronics Korea', pol: 'JPNGO', pod: 'KRPUS', etd: '2026-01-30', eta: '2026-02-05', cargo_type: 'LCL', commodity: '가전부품', quantity: 50, weight: 8000, volume: 45, incoterms: 'CIF', status: 'REQUESTED', remarks: '', created_at: '2026-01-25' },
  { id: 3, co_number: 'CO20260124-0001', order_type_code: 'EXP-AIR', order_type_name: 'Export Air', biz_type: 'FORWARDING', customer_code: 'HYUNDAI', customer_name: '현대자동차', shipper_name: 'Hyundai Motor', consignee_name: 'Hyundai Europe', pol: 'ICN', pod: 'FRA', etd: '2026-01-26', eta: '2026-01-27', cargo_type: 'AIR', commodity: '자동차부품', quantity: 200, weight: 3500, volume: 25, incoterms: 'DAP', status: 'PROCESSING', remarks: '긴급배송', created_at: '2026-01-24' },
  { id: 4, co_number: 'CO20260123-0001', order_type_code: 'DOM-TRK', order_type_name: 'Domestic Truck', biz_type: 'DOMESTIC', customer_code: 'SK', customer_name: 'SK하이닉스', shipper_name: 'SK Hynix', consignee_name: 'SK Hynix 이천공장', pol: '서울', pod: '이천', etd: '2026-01-25', eta: '2026-01-25', cargo_type: 'BULK', commodity: '반도체장비', quantity: 10, weight: 15000, volume: 80, incoterms: 'EXW', status: 'COMPLETED', remarks: '', created_at: '2026-01-23' },
  { id: 5, co_number: 'CO20260122-0001', order_type_code: 'EXP-FCL', order_type_name: 'Export FCL', biz_type: 'FORWARDING', customer_code: 'POSCO', customer_name: '포스코', shipper_name: 'POSCO', consignee_name: 'POSCO America', pol: 'KRPUS', pod: 'USNYC', etd: '2026-02-01', eta: '2026-02-20', cargo_type: 'FCL', commodity: '철강제품', quantity: 500, weight: 150000, volume: 600, incoterms: 'CFR', status: 'DRAFT', remarks: '', created_at: '2026-01-22' },
];
...
export default function CustomerOrderPage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/oms/customer-order?id=${id}`, {
        method: 'DELETE',
      });
  ...
  return (
      <PageLayout title="고객 오더 관리 (C/O)" subtitle="Logis > OMS > Customer Order" showCloseButton={false}>
      <main className="p-6">
      ...
      <div className="card mb-6">
        ...
        <div className="p-4">
          <div className="grid grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1 text-[var(--foreground)]">업무구분</label>
              <select
        ...
        <div className="p-4 border-t border-[var(--border)] flex justify-center gap-2">
          ...
          <button onClick={() => setSearchFilters({ status: '', bizType: '', fromDate: '', toDate: '' })} className="px-6 py-2 bg-[var(--surface-100)] border border-[var(--border)] rounded-lg hover:bg-[var(--surface-200)]">초기화</button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="table">
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {loading ? (
                <tr>
                  <td colSpan={13} className="px-4 py-8 text-center text-[var(--foreground)]/60">
                    로딩 중...
                  </td>
                </tr>
              ) : orders.length === 0 ? (
                ...
                orders.map((order, index) => (
                  <tr key={order.id} className="hover:bg-[var(--surface-200)]/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-[#E8A838] text-center">{order.co_number}</td>
                    ...
                    <td className="px-4 py-3 text-sm text-center">{order.commodity}</td>
                    <td className="px-4 py-3 text-sm text-center">{order.quantity?.toLocaleString()}</td>
                    <td className="px-4 py-3 text-sm text-center">{order.weight?.toLocaleString()}</td>
                    <td className="px-4 py-3 text-center">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-xs text-[var(--foreground)] mb-1">업무 구분</label>
                    <select
